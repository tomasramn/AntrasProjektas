...
{
	list<studentas>::iterator it = find_if(studinfo.begin(), studinfo.end(), arlygu5); // iteratorius, rodantis kurioje vietoje egzaminas lygus 5
...
void VectTestingClass(vector<studentasc>& vargsiukai, vector <studentasc>& studinfo, int kiekis) {
	...
	try {
		galutinisklases(studinfo, kiekis);
	}
	catch (exception& e) {
	...
	try {
		...
		// 2 budas: naudojamas su surusiuok3klase()
